...
public static class UI
{
    ...
    public static void AzurirajReceptPoKategorijiISastojkuUI(IRecipeService recipeService, IIngredientService ingredientService)
    {
        Console.Write("Unesite naziv kategorije: ");
        string kategorija = Console.ReadLine()?.Trim();

        if (string.IsNullOrEmpty(kategorija))
        {
            Console.WriteLine("Morate unijeti kategoriju.");
            return;
        }

        Console.Write("Unesite naziv sastojka: ");
        string nazivSastojka = Console.ReadLine()?.Trim();
        if (string.IsNullOrEmpty(nazivSastojka))
        {
            Console.WriteLine("Morate unijeti sastojak.");
            return;
        }

        var sastojak = ingredientService.GetIngredientByName(nazivSastojka);

        if (sastojak == null)
        {
            Console.WriteLine("Sastojak nije pronađen.");
            return;
        }
        try
        {
            recipeService.AzurirajReceptPoKategorijiISastojku(kategorija, sastojak);
            var azuriraniRecepti = recipeService.GetSviRecepti();
            foreach (var recept in azuriraniRecepti)
            {
                Console.WriteLine(recept);
            }
        }
