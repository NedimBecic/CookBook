...
namespace Services
{
    public class RecipeService : IRecipeService
    {
        ...
        public void AzurirajReceptPoKategorijiISastojku(string kategorija, Ingredient sastojak)
        {
            if(string.IsNullOrEmpty(kategorija))
            {
                throw new Exception($"Kategorija nije unesena.");
            } else if(sastojak == null)
            {
                throw new Exception($"Sastojak nije unesen.");
            }
            //Mijenjamo kategoriju (npr. u "Nije moguce spremiti") za sve recepte ako sadrze odredjeni sastojak (npr. chili)
            var sviRecepti = GetSviRecepti();
            for (int i = 0; i < sviRecepti.Count; i++)
            {
                if (sviRecepti[i].Kategorija == kategorija && sviRecepti[i].Sastojci.ContainsKey(sastojak.Id))
                {
                    sviRecepti[i].Kategorija = "Nije moguce spremiti.";
                }
            }
